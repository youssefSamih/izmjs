...
import { isExcluded } from '@helpers/utils';
...
async function seedIAMs() {
  ...
  const all$ = config.files.server.iam.map(async (iamFilePath: string) => {
    ...
    if (Array.isArray(m.routes)) {
      const routes$ = m.routes.map(async (route: { methods: { [x: string]: any }; path: any }) => {
...
async function seedRoles() {
  ...
  const cache: any = {};
  const iams = []
    .concat(...roles.map((r: { iams: any }) => r.iams))
  ...
  const promises = roles.map(async (role: { iams: any[]; name: any }) => {
    let list = Array.isArray(role.iams) ? role.iams : [];
    list = role.iams.map((iam) => cache[iam]).filter(Boolean);

    try {
      let r: any = await RoleModel.findOne({ name: role.name });
      ...
      if (!r) {
        ...
        list.forEach((iam) => {
          const found = r.iams.find((item: { toString: () => any }) => item.toString() === iam.id);
  ...
  const iamsFiles = config.files.server.iams.map(async (iamFilePath: string) => {
    let m;
    try {
